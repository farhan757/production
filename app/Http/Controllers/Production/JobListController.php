...
class JobListController extends Controller
{
	...
	public function deleteJob(Request $request)
	{
		...
		try {
			//code...
			DB::table("production_data")->where("production_data.id", "=", $request->id)->delete();
			DB::commit();
	...
	{
		# code...
		$status_warehouse = DB::table('production_data')->where('id', $request->id)->first();
		if ($status_warehouse->status_warehouse == 0) {
			$this->updateTask($request->id, $this->submitJobListId, 11, $request->note);
			return response()->json([
				'status' => 1,
			...
			return response()->json([
				'status' => 2,
				'message' => "Canot Cancel, because data has been production"
			]);
		}
	}
