...
class TaskCSController extends Controller
{
    //
    function show()
    {
    	...
    	->paginate(10);
    	return view('CustomerService.Task.list')->with('list',$list);
    }

    function showformupdate($id)
    {
    	$liststatus = DB::table('desc_status')->get();
    	$data = DB::table('customer_care')->where('id','=',$id)->first();
    	return view('CustomerService.Task.formupdate')->with('data',$data)->with('liststatus',$liststatus);
    }

    function updatestatus(Request $request, $id)
    {
    	$ket = $request->input('inputMessage');
    	if($ket==null) $ket='';
    	if($request->input('selectStatus')==5)
    		$complete=1;
    	else $complete=0;
    	DB::table('customer_care')->where('id','=',$id)->update([
    		'update_dt'=>Carbon::now(),
    		'status'=>$request->input('selectStatus'),
    		'message'=>$ket,
    		'complete'=>$complete
    	]);
    	DB::table('status_cs')->insert([
    		'id_cs'=>$id,
    		'id_status'=>$request->input('selectStatus'),
    		'uid_updated'=>Auth::id(),
    		'ket'=>$ket,
    		'update_dt'=>Carbon::now()
    	]);

    	 return view('CustomerService.Task.landing');
    }
    ...
    function showdetail($id)
    {
    	...
    	->where('id_cs','=',$id)->get();
    	return view('CustomerService.Task.detail')->with('data',$data)->with('list',$timeline)->with('issama',$issama)->with('files',$files2);
    }
}
